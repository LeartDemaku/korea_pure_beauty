...
if "pending_verification_code_test" not in st.session_state:
...
<style>
    ...
    html[data-theme="light"] .brand-subtext { color: #64748b !important; }
    html[data-theme="dark"] .brand-subtext { color: #a4b0be !important; }
...
def render_interactive_zoom_viewer(image_path: str, product_name: str, height: int = 560):
    ...
    <script>
        ...
        // Desktop Double click quick zoom
...
# ==========================================
# 🌸 APLIKACIONI I DEDIKUAR I KLIENTIT
# ==========================================
def render_client_app():
    ...
    with st.sidebar:
        ...
        <div class="profile-card">
            ...
            <div class="profile-user-text">
                Mirësevini: <strong class="profile-user-name">{username_display}</strong>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if st.session_state.authenticated:
            ...
            with st.popover("🔑 Ndërro Fjalëkalimin", use_container_width=True):
                ...
                cp_old = st.text_input("Fjalëkalimi Aktual", type="password", key="side_cli_old_p")
                cp_new = st.text_input("Fjalëkalimi i Ri", type="password", key="side_cli_new_p", placeholder="Të paktën 6 karaktere")
                cp_conf = st.text_input("Konfirmo Fjalëkalimin e Ri", type="password", key="side_cli_conf_p")
                if st.button("Ruaj Fjalëkalimin", key="side_btn_save_cli_p", use_container_width=True, type="primary"):
                    identifier = user.get('email') or user.get('username')
                    ok, msg = auth.change_password_with_current(identifier, cp_old, cp_new, cp_conf)
                    if ok:
                        st.success("🎉 " + msg)
                    else:
                        st.error(msg)
    ...
    # ==========================================
    top_col_brand, top_col_auth = st.columns([2.6, 1.4])
    with top_col_brand:
        fav_icon_uri = get_favicon_data_uri()
        if fav_icon_uri:
            st.markdown(f"""
            <div class="top-header-brand-wrap" style="display: flex; align-items: center; gap: 12px; padding: 2px 0;">
                <img src="{fav_icon_uri}" alt="KPB" style="width: 38px; height: 38px; border-radius: 50%; box-shadow: 0 3px 10px rgba(255, 117, 140, 0.22); border: 1px solid rgba(255, 184, 198, 0.5); object-fit: cover;" />
                <div style="line-height: 1.25;">
                    <div style="font-size: 1.05rem; font-weight: 800; color: #ff758c; letter-spacing: 0.5px;">
                        KOREA PURE BEAUTY
                    </div>
                    <div class="brand-subtext" style="font-size: 0.82rem; font-weight: 600; opacity: 0.95;">
                        ✨ Rituali Luksoz për një Lëkurë të Shëndetshme & me Shkëlqim Natyral
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
            ...
            st.markdown(
                "<div style='padding: 8px 0; font-size: 0.95rem; font-weight: 700; color: #ff758c;'>"
                "🌸 KOREA PURE BEAUTY <span class='brand-subtext' style='font-weight: 600; font-size: 0.85rem; opacity: 0.95;'>| ✨ Rituali Luksoz për një Lëkurë të Shëndetshme & me Shkëlqim Natyral</span>"
                "</div>",
                unsafe_allow_html=True
            )
    with top_col_auth:
        if st.session_state.authenticated:
            u_col, out_col = st.columns([1.6, 1])
            with u_col:
                st.markdown(f"<div style='padding: 8px 0; text-align: right; font-weight: 700; font-size: 0.9rem;'>👤 {username_display}</div>", unsafe_allow_html=True)
            with out_col:
                if st.button("🚪 Dil", key="top_client_logout", use_container_width=True, type="secondary"):
                    st.session_state.authenticated = False
                    st.session_state.current_user = None
                    st.session_state.selected_product_id = None
                    st.session_state.current_page = "shop"
            ...
            with st.popover("🔑 Kyçu / Regjistrohu", use_container_width=True):
                pending_email = st.session_state.get("pending_verification_email")

                if pending_email:
                    # ==========================================
                    # 🔐 EKRANI I VERIFIKIMIT ME KOD (EMAIL)
                    # ==========================================
                    st.markdown("#### ✉️ Verifikimi i Llogarisë")
                    st.write("Kodi 6-shifror i verifikimit iu dërgua te:")
                    st.markdown(f"**`{pending_email}`**")

                    test_code = st.session_state.get("pending_verification_code_test")
                    if not email_service.is_smtp_configured() and test_code:
                        st.info(f"💡 **Kodi i testimit lokal:** `{test_code}`\n\n*(Ju lutem plotësoni skedarin `.env` me kredencialet tuaja Gmail SMTP për dërgim real në email)*")

                    code_inp = st.text_input("Vendosni Kodin 6-shifror nga Email-i", max_chars=6, key="top_v_code", placeholder="p.sh. 123456")

                    v_col1, v_col2 = st.columns([1.4, 1])
                    with v_col1:
                        if st.button("✅ Aktivizo & Hyr", key="btn_confirm_code", use_container_width=True, type="primary"):
                            if code_inp and len(code_inp.strip()) == 6:
                                v_ok, v_msg = auth.verify_client_code(pending_email, code_inp.strip())
                                if v_ok:
                                    u = database.get_user_by_email(pending_email)
                                    st.session_state.authenticated = True
                                    st.session_state.current_user = u
                                    st.session_state.pending_verification_email = None
                                    st.session_state.pending_verification_code_test = None
                                    st.query_params["user"] = u['username']
                                    st.success("🎉 Llogaria u aktivizua me sukses! Mirësevini në dyqan.")
                                    st.rerun()
                                else:
                                    st.error(v_msg)
                            else:
                                st.warning("Ju lutem shkruani kodin 6-shifror.")

                    with v_col2:
                        if st.button("🔄 Ridërgo Kodin", key="btn_resend_code", use_container_width=True, type="secondary"):
                            r_ok, r_msg, new_code = auth.resend_client_code(pending_email)
                            st.session_state.pending_verification_code_test = new_code
                            if r_ok:
                                st.success("Kodi i ri u dërgua në email!")
                            else:
                                st.warning(r_msg)
                            st.rerun()

                    st.write("")
                    if st.button("⬅️ Kthehu / Ndrysho Email-in", key="btn_cancel_verify", use_container_width=True, type="secondary"):
                        st.session_state.pending_verification_email = None
                        st.session_state.pending_verification_code_test = None
                        st.rerun()

                else:
                    tab_l, tab_r, tab_p = st.tabs(["🔑 Hyr në Llogari", "✨ Krijo Llogari", "🔄 Ndërro Fjalëkalimin"])
                            ...
                            if lu and lp:
                                ...
                                if succ:
                                    ...
                                    if isinstance(res, str) and res.startswith("UNVERIFIED:"):
                                        ...
                                        st.session_state.pending_verification_email = u_email
                                        st.warning("Llogaria juaj nuk është verifikuar ende. Ju lutem vendosni kodin.")
                                        st.rerun()
                                    else:
                                        st.error(res)
                            else:
                    ...
                    # ------------------------------------------
                    # 2. REGJISTRIM I RI I KLIENTIT
                    # ------------------------------------------
                    with tab_r:
                        st.markdown("#### Regjistrim i Ri Klienti")
                        r_name = st.text_input("1. Emri dhe Mbiemri *", key="top_r_name", placeholder="p.sh. Sara Hoxha")
                        r_email = st.text_input("2. Email Adresa *", key="top_r_email", placeholder="p.sh. sara.hoxha@gmail.com")
                        r_pass = st.text_input("3. Fjalëkalimi *", type="password", key="top_r_pass", placeholder="Të paktën 6 karaktere")
                        r_confirm = st.text_input("4. Konfirmo Fjalëkalimin *", type="password", key="top_r_confirm", placeholder="Rishkruani fjalëkalimin")

                        if st.button("🌸 Regjistrohu & Merr Kodin", key="top_btn_r", use_container_width=True, type="primary"):
                            succ, msg, code = auth.register_client(r_name, r_email, r_pass, r_confirm)
                            if succ:
                                st.session_state.pending_verification_email = r_email.strip().lower()
                                st.session_state.pending_verification_code_test = code
                                st.rerun()
                            else:
                                st.error(msg)

                    # ------------------------------------------
                    # 3. NDËRRO / RIVENDOS FJALËKALIMIN
                    # ------------------------------------------
                    with tab_p:
                        st.markdown("#### 🔄 Ndërro Fjalëkalimin")
                        reset_mode = st.radio(
                            "Mënyra e ndërrimit:",
                        ...
                        if "Me Kod në Email" in reset_mode:
                            if "c_reset_sent" not in st.session_state:
                                st.session_state.c_reset_sent = False
                            if "c_reset_target" not in st.session_state:
                                st.session_state.c_reset_target = ""

                            if not st.session_state.c_reset_sent:
                                r_ident = st.text_input("Email ose Username i llogarisë", key="c_reset_ident_inp", placeholder="p.sh. emri@gmail.com")
                                if st.button("📩 Dërgo Kodin e Sigurisë", key="c_btn_send_reset_code", use_container_width=True, type="primary"):
                                    if r_ident:
                                        ok, msg, target_email, code = auth.request_password_reset_code(r_ident)
                                        if ok:
                                            st.session_state.c_reset_sent = True
                                            st.session_state.c_reset_target = r_ident
                                            st.session_state.c_reset_code_test = code
                                ...
                                target_id = st.session_state.c_reset_target
                                st.info(f"Kodi i sigurisë iu dërgua llogarisë: **{target_id}**")
                                test_c = st.session_state.get("c_reset_code_test")
                                if not email_service.is_smtp_configured() and test_c:
                                    st.caption(f"💡 *Kodi lokal i testimit:* `{test_c}`")

                                r_code = st.text_input("Kodi 6-shifror nga Email-i", max_chars=6, key="c_reset_code_inp", placeholder="p.sh. 123456")
                                r_new_p = st.text_input("Fjalëkalimi i Ri", type="password", key="c_reset_new_p", placeholder="Të paktën 6 karaktere")
                                r_conf_p = st.text_input("Konfirmo Fjalëkalimin e Ri", type="password", key="c_reset_conf_p", placeholder="Rishkruani fjalëkalimin")

                                c_btn1, c_btn2 = st.columns([1.5, 1])
                                with c_btn1:
                                    if st.button("💾 Ruaj Fjalëkalimin", key="c_btn_confirm_reset_p", use_container_width=True, type="primary"):
                                        r_ok, r_msg = auth.confirm_password_reset_code(target_id, r_code, r_new_p, r_conf_p)
                                        if r_ok:
                                            st.session_state.c_reset_sent = False
                                            st.session_state.c_reset_target = ""
                                            st.success("🎉 Fjalëkalimi u ndryshua me sukses! Mund të hyni te 'Hyr në Llogari'.")
                                            st.rerun()
                                        else:
                                            st.error(r_msg)
                                with c_btn2:
                                    if st.button("⬅️ Kthehu", key="c_btn_back_reset", use_container_width=True, type="secondary"):
                                        st.session_state.c_reset_sent = False
                                        st.session_state.c_reset_target = ""
                                        st.rerun()

                        else:
                            curr_id = st.text_input("Email ose Username", key="c_chg_ident_inp", placeholder="p.sh. emri@gmail.com")
                            curr_old_p = st.text_input("Fjalëkalimi Aktual", type="password", key="c_chg_old_p")
                            curr_new_p = st.text_input("Fjalëkalimi i Ri", type="password", key="c_chg_new_p", placeholder="Të paktën 6 karaktere")
                            curr_conf_p = st.text_input("Konfirmo Fjalëkalimin e Ri", type="password", key="c_chg_conf_p", placeholder="Rishkruani fjalëkalimin")

                            if st.button("💾 Përditëso Fjalëkalimin", key="c_btn_update_direct_p", use_container_width=True, type="primary"):
                                chg_ok, chg_msg = auth.change_password_with_current(curr_id, curr_old_p, curr_new_p, curr_conf_p)
                                if chg_ok:
                                    st.success("🎉 " + chg_msg)
                                else:
                                    st.error(chg_msg)
    ...
    # Nëse po shikon detajet e produktit
    ...
    if current_page == "shop":
        ...
        if not all_p:
            st.info("💡 Dyqani po përgatitet me produkte të reja. Ju lutem rikthehuni së shpejti!")
            st.stop()
        ...
        if st.session_state.get("last_order"):
            ...
            with c_right:
                with st.container(border=True):
                    ...
                    b_name = st.text_input("Emri dhe Mbiemri *", placeholder="p.sh. Blerta Krasniqi")
                    b_phone = st.text_input("Numri i Telefonit *", placeholder="p.sh. +383 44 123 456")
                    b_city = st.text_input("Qyteti *", placeholder="Shkruani qytetin tuaj (p.sh. Prishtinë, Tiranë, Ferizaj, etj.)")
                    b_addr = st.text_area("Adresa e plotë e dërgesës *", placeholder="Rruga, Numri i banesës/shtëpisë...")
