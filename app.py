...
def render_main_app():
    ...
    # ----------------------------------------------------
        ...
        st.subheader("📦 Lista e Artikujve në Inventar")
        ...
        if inv_search and inv_search.strip():
            tokens = normalize_albanian_text(inv_search).split()
            filtered_products = [
                p for p in products
                if all(
                    tk in normalize_albanian_text(f"{p.get('name','')} {p.get('brand','')} {p.get('category','')} {p.get('skin_type','')} {p.get('description','')}")
                    for tk in tokens
                )
            ]
